...
(define-test-case "scratch common test"
  ("get-param  test"
   ...
   ("generate-id&action&language test"
    (for-each
     (lambda (id)
       (parameterize ((session (make-scratch-session))
                      (languages '()))
         ...
         (assert-each-list-elem
          (lambda (expected . args)
            (call-with-values (cut apply generate-id&action&language args)
              (lambda actual (list expected actual))))
          :lists `(((,id default #f ()))
                   ((,id default #f ()) :new-session #f)
                   ((0 default #f ()) :new-session #t)
                   ((,id default #f ()) :new-session #t :new-session #f)
                   ((0 default #f ()) :new-session #f :new-session #t)
                   ((,id default #f ()) :new-session #f :new-session #f)
                   ((,id abc #f ()) :action abc)
                   ((,id def #f ()) :action abc :action def)
                   ((,id default #f (a)) a)
                   ((,id default #f (a)) :new-session #f a)
                   ((,id default #f (a)) a :new-session #f)
                   ((0 default #f (a)) a :new-session #t)
                   ((,id abc #f (a)) a :action abc)
                   ((,id def #f (a :other)) a :action abc :other :action def)
                   ((,id abc #f ()) :new-session #t :action abc :new-session #f)
                   ((0 abc #f (a b c)) a :action abc b :new-session #t c)
                   ((,id default "ja" ()) :language "ja")
                   ((,id default "en" ()) :language "ja" :language "en")
                   ((0 abc "ja" (a b c)) a :action abc b
                                           :new-session #t c
                                           :language "ja")))))
     '(#f 10 100)))
   )
